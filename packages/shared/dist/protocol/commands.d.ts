...
export type ChatCommand = ClientCommand<ChatRequest> & {
    type: 'chat';
};
...
export type CancelCommand = ClientCommand<{
    sessionId: string;
}> & {
    type: 'cancel';
};
export type PingCommand = ClientCommand<Record<string, never>> & {
    type: 'ping';
};
...
export type ErrorCode = 'auth' | 'rate_limit' | 'provider_unavailable' | 'no_knowledge_base' | 'bad_request' | 'internal';
